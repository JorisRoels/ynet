...
# 2D Y-Net model for unsupervised domain adaptation
...
#
#     def __init__(self, in_channels=1, out_channels=2, feature_maps=64, levels=4, batch_norm=True, lambda_rec=1e-3,
#                  dropout=0.0):
...
#         # reconstruction decoder
#         self.reconstruction_decoder = UNetDecoder2D(out_channels=in_channels, feature_maps=feature_maps, levels=levels,
#                                                     batch_norm=batch_norm, skip_connections=False)
#
#     def forward(self, inputs):
#
#         # encoder
...
#                 a, p, r, f, j, d = validate(self, test_data, test_labels,
#                                             train_loader_source_labeled.dataset.input_shape[-2:],
...
#
#         # encoder
#         self.encoder = UNetEncoder3D(in_channels=in_channels, feature_maps=feature_maps, levels=levels,
#                                      batch_norm=batch_norm, dropout=dropout)
#
#         # segmentation decoder
...
#             x_pred = torch.cat((x_src_pred, x_tar_pred), dim=0).reshape(-1, x_src_pred.size(2), x_src_pred.size(3),
#                                                                         x_src_pred.size(4))
#             y_pred = F.softmax(torch.cat((y_src_pred, y_tar_pred), dim=0), dim=1)[:, 1:2, :, :].data.reshape(-1,
#                                                                                                              y_src_pred.size(
#                                                                                                                  2),
#                                                                                                              y_src_pred.size(
#                                                                                                                  3),
