...
from neuralnets.util.tools import module_to_device, tensor_to_device, log_scalars, log_images_2d, augment_samples
...
class YNet2D(nn.Module):
    ...
    def test_epoch(self, loader_src, loader_tar, loss_seg_fn, loss_rec_fn, epoch, writer=None, write_images=False,
        ...
        for i, data in enumerate(zip(loader_src, loader_tar)):
            ...
            y_src_preds.append(F.softmax(y_src_pred, dim=1).data.cpu().numpy()[:, 1, ...])
            y_tar_preds.append(F.softmax(y_tar_pred, dim=1).data.cpu().numpy()[:, 1, ...])
            ys_src.append(y_src[:, 0, ...].cpu().numpy())
            ys_tar.append(y_tar[:, 0, ...].cpu().numpy())
