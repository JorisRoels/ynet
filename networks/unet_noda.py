...
# 2D U-Net model
class UNetNoDA2D(UNet2D):

    def __init__(self, in_channels=1, coi=(0, 1), feature_maps=64, levels=4, norm='instance', lambda_dc=1e0,
                 dropout=0.0, activation='relu'):
        super(UNetNoDA2D, self).__init__(in_channels=in_channels, coi=coi, feature_maps=feature_maps, levels=levels,
                                         norm=norm, activation=activation, dropout_enc=dropout, dropout_dec=dropout)
    ...
    def train_epoch(self, loader_src, loader_tar_ul, loader_tar_l, optimizer, epoch, augmenter=None, print_stats=1,
            ...
            if i % print_stats == 0:
                print(
                    ...
                    % (datetime.datetime.now(), epoch, i, len(loader_src.dataset) / loader_src.batch_size,
                       loss_seg_src_cum / cnt, loss_seg_tar_cum / cnt, total_loss_cum / cnt))
        ...
        print('[%s] Training Epoch %4d - Loss seg src: %.6f - Loss seg tar: %.6f - Loss: %.6f' % (
            datetime.datetime.now(), epoch, loss_seg_src_avg, loss_seg_tar_avg, total_loss_avg))
        ...
        if writer is not None:
            ...
            log_scalars([loss_seg_src_avg, loss_seg_tar_avg, total_loss_avg],
                        ['train/' + s for s in ['loss-seg-src', 'loss-seg-tar', 'total-loss']], writer, epoch=epoch)

            # log images if necessary
            if write_images:
                y_src_pred = F.softmax(y_src_pred, dim=1)[:, 1:2, :, :].data
                log_images_2d([x_src.data, y_src.data, y_src_pred],
                              ['train/' + s for s in ['src/x', 'src/y', 'src/y-pred']], writer, epoch=epoch)
                if loader_tar_l is not None:
    ...
    def test_epoch(self, loader_src, loader_tar_ul, loader_tar_l, epoch, writer=None, write_images=False, device=0):
        ...
        loss_seg_tar_avg = loss_seg_tar_cum / cnt
        total_loss_avg = total_loss_cum / cnt
        print('[%s] Testing Epoch %4d - Loss seg src: %.6f - Loss seg tar: %.6f - Loss: %.6f' % (
            datetime.datetime.now(), epoch, loss_seg_src_avg, loss_seg_tar_avg, total_loss_avg))
        ...
        if writer is not None:
            ...
            # always log scalars
            log_scalars(
                [loss_seg_src_avg, loss_seg_tar_avg, total_loss_avg, np.mean(js, axis=0),
                 *(np.mean(ams, axis=0))], ['test/' + s for s in
                                            ['loss-seg-src', 'loss-seg-tar', 'total-loss', 'jaccard', 'accuracy',
            ...
            if write_images:
                y_src_pred = F.softmax(y_src_pred, dim=1)[:, 1:2, :, :].data
                y_tar_l_pred = F.softmax(y_tar_l_pred, dim=1)[:, 1:2, :, :].data
                log_images_2d([x_src.data, y_src.data, y_src_pred, x_tar_l.data, y_tar_l, y_tar_l_pred],
                              ['test/' + s for s in
                               ['src/x', 'src/y', 'src/y-pred', 'tar/x-l', 'tar/y-l', 'tar/y-l-pred']], writer,
                              epoch=epoch)
