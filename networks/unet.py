...
class UNetEncoder2D(nn.Module):
    ...
    def __init__(self, in_channels=1, feature_maps=64, levels=4, batch_norm=True, dropout=0.0):
        ...
        for i in range(levels):
            ...
            # convolutional block
            conv_block = UNetConvBlock2D(in_features, out_features, batch_norm=batch_norm, dropout=dropout)
        ...
        # center (lowest) block
        self.center_conv = UNetConvBlock2D(2**(levels-1) * feature_maps, 2**levels * feature_maps, batch_norm=batch_norm, dropout=dropout)
...
class UNetDecoder2D(nn.Module):
    ...
    def __init__(self, in_channels=1, out_channels=2, feature_maps=64, levels=4, skip_connections=True, batch_norm=True):
        ...
        for i in range(levels):
            ...
            # convolutional block
            if skip_connections:
                conv_block = UNetConvBlock2D(2**(levels-i) * feature_maps, 2**(levels-i-1) * feature_maps, batch_norm=batch_norm)
            else:
                conv_block = UNetConvBlock2D(2**(levels-i-1) * feature_maps, 2**(levels-i-1) * feature_maps, batch_norm=batch_norm)
            self.features.add_module('convblock%d' % (i + 1), conv_block)
...
class UNet2D(nn.Module):
    ...
    def __init__(self, in_channels=1, out_channels=2, feature_maps=64, levels=4, batch_norm=False, skip_connections=True):
        ...
        # contractive path
        self.encoder = UNetEncoder2D(in_channels, feature_maps=feature_maps, levels=levels, batch_norm=batch_norm)
        # expansive path
        self.decoder = UNetDecoder2D(in_channels, out_channels, feature_maps=feature_maps, levels=levels, skip_connections=skip_connections, batch_norm=batch_norm)
    ...
    # trains the network
    def train_net(self, train_loader, test_loader, loss_fn, epochs=100, optimizer=None, scheduler=None, test_freq=1, print_stats=1, log_dir=None, write_images_freq=1):
...
class UNetEncoder3D(nn.Module):
    ...
    def __init__(self, in_channels=1, feature_maps=64, levels=4, batch_norm=True, dropout=0.0):
        ...
        for i in range(levels):
            ...
            # convolutional block
            conv_block = UNetConvBlock3D(in_features, out_features, batch_norm=batch_norm, dropout=dropout)
            self.features.add_module('convblock%d' % (i + 1), conv_block)
        ...
        # center (lowest) block
        self.center_conv = UNetConvBlock3D(2**(levels-1) * feature_maps, 2**levels * feature_maps, batch_norm=batch_norm, dropout=dropout)
...
class UNetDecoder3D(nn.Module):
    ...
    def __init__(self, in_channels=1, out_channels=2, feature_maps=64, levels=4, skip_connections=True, batch_norm=True, dropout=0.0):
        ...
        for i in range(levels):
            ...
            # convolutional block
            if skip_connections:
                conv_block = UNetConvBlock3D(2**(levels-i) * feature_maps, 2**(levels-i-1) * feature_maps, batch_norm=batch_norm, dropout=dropout)
            else:
                conv_block = UNetConvBlock3D(2**(levels-i-1) * feature_maps, 2**(levels-i-1) * feature_maps, batch_norm=batch_norm, dropout=dropout)
            self.features.add_module('convblock%d' % (i + 1), conv_block)
...
class UNet3D(nn.Module):
    ...
    def __init__(self, in_channels=1, out_channels=2, feature_maps=64, levels=4, batch_norm=True, skip_connections=True):
        ...
        # contractive path
        self.encoder = UNetEncoder3D(in_channels, feature_maps=feature_maps, levels=levels, batch_norm=batch_norm)
        # expansive path
        self.decoder = UNetDecoder3D(in_channels, out_channels, feature_maps=feature_maps, levels=levels, skip_connections=skip_connections, batch_norm=batch_norm)
...
def unet_from_encoder_decoder(encoder, decoder, vol=False):

    if vol:
        net = UNet3D(in_channels=encoder.in_channels, out_channels=decoder.out_channels, feature_maps=encoder.feature_maps, levels=encoder.levels)
    else:
