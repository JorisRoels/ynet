...
# generate an input and target sample of certain shape from the embl embryo dataset
def sample_labeled_input_embryo(data, labels, input_shape):

    # randomize seed
    np.random.seed()

    # generate random position
    x = np.random.randint(0, data.shape[1]-input_shape[0]+1)
    y = np.random.randint(0, data.shape[2]-input_shape[1]+1)
    z = np.random.randint(0, data.shape[3]-input_shape[2]+1)

    # extract input and target patch
    input = data[:, x:x+input_shape[0], y:y+input_shape[1], z:z+input_shape[2]]
    if len(labels)>0:
        target = labels[x:x+input_shape[0], y:y+input_shape[1], z:z+input_shape[2]][np.newaxis, ...]
    else:
        target = []

    if input_shape[0]>1:
        return copy.copy(input), copy.copy(target)
    else: # lose the z-channel of the targets if the inputs are 2D
        return copy.copy(input[:, 0, ...]), copy.copy(target[:, 0, ...])
...
# generate an input sample of certain shape from the embl embryo dataset
def sample_unlabeled_input_embryo(data, input_shape):
