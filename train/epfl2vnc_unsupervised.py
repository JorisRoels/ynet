...
src_train = StronglyLabeledVolumeDataset(os.path.join(args.data_dir, 'EM/EPFL/data.tif'),
                                         os.path.join(args.data_dir, 'EM/EPFL/labels.tif'),
                                         input_shape=input_shape, augmenter=augmenter, preprocess='unit', scaling=scaling)
tar_train = UnlabeledVolumeDataset(os.path.join(args.data_dir, 'EM/VNC/data_larger.tif'), input_shape=input_shape,
                                   preprocess='unit')
tar_test = StronglyLabeledVolumeDataset(os.path.join(args.data_dir, 'EM/VNC/data.tif'),
                                        os.path.join(args.data_dir, 'EM/VNC/mito_labels.tif'),
                                        input_shape=input_shape, preprocess='unit')
...
net = YNet(feature_maps=args.fm, levels=args.levels, dropout=args.dropout, lambda_rec=args.lambda_rec)
...
net.train_net(train_loader_source=src_train_loader, train_loader_target=tar_train_loader,
              test_data=tar_test.data/255, test_labels=tar_test.labels,
...
"""
validate(net, tar_test.data/255, tar_test.labels, args.input_size, batch_size=args.test_batch_size,
